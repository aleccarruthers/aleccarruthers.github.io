...
authors:
  - name: Alec Carruthers
    url: "https://www.linkedin.com/in/alec-carruthers/"
    affiliations:
      name: MIT

bibliography: 2025-08-17-generative-ai.bib
...
Some of the most popular generative models used today were trained with diffusion, autoregression, normalizing flows, or flow matching.
Each of these methods, at their core, essentially maximize the likelihood of the data distribution. They all achieve this in somewhat different
ways, but maximizing likelihood of the data nonetheless.

What does this mean for us? Is this a bad thing?

My view is that this is perfectly fine where we want models to synthesize concepts and ideas within the support of the data distribution.

The exact text Gemini or ChatGPT output may not have been observed within the data distribution, especially if you ask it an obscure enough question.
These models seem to be composing tokens they have seen in the training data into hopefully coherent strings / concepts.
Similarly, "unique" images produced by Stable-Diffusion could just be the result of interpolating between high density regions in the data distribution.

What happens though when we'd like to use the model for research and making scientific discoveries? Let's consider designing novel materials as an example.
Even though we have collected large datasets of different types of materials from alloys, 2d materials, plastics, etc., surely this
does not encompass all possible materials that could be created in the universe. Thus, a generative model trained on said data should do
well at representing molecules / atoms / materials that we already have an understanding of and potentially make some "novel"
discoveries by connecting components in the data distribution.

However, based on its underlying training objective, it does not seem plausible to me that it will make discoveries outside of the support of the data distribution.
Novelty does not exist only in connecting different topics we already know about, but also in the space of exploring beyond what is currently known.

At the highest of high levels, and I am not sure how this would be practically achieved (yet), but it seems to me like we'd want generative models to be
trained in a way to discover and represent the topology of distributions as opposed to learning how to map between generally fixed distributions (noise and data).
Again, hypothetically, it seems like a model that could identify promising regions of high-dimensional space (with some of them being made up by existing data)
would be beneficial for applications in scientific discovery compared to approaches transporting between two preset distributions.
